  ...
  methods: {
    ...
    async submitForm(params) {
      try {
        ...
        const updateResult = await db.collection("uni-id-users").where({
          ...
          avatar_file: {
            extname: "png",
            name: "defaultAvatar",
            url: "https://mp-55788fb7-2392-4142-8486-dfaba6fb925c.cdn.bspapp.com/cloudstorage/default.png"
          },
          user_role: "user"
        });
        console.log(updateResult);
        if (updateResult.result.updated === 1) {
          console.log("用户 avatar_file 更新成功");
        } else {
          console.error("用户 avatar_file 更新失败: 未找到匹配的用户记录");
        }
      } catch (error) {
        this.$refs.captcha.getImageCaptcha();
        console.error("注册或更新失败:", error);
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return {
    ...
    t: common_vendor.sr("captcha", "3f414d9c-10,3f414d9c-9"),
    v: common_vendor.o(($event) => $data.formData.captcha = $event),
    w: common_vendor.p({
      scene: "register",
      modelValue: $data.formData.captcha
    }),
    x: common_vendor.sr("agreements", "3f414d9c-11,3f414d9c-0"),
    y: common_vendor.p({
      scope: "register"
    }),
    ...
    D: common_vendor.sr("form", "3f414d9c-0"),
    E: common_vendor.p({
      value: $data.formData,
      rules: $data.rules,
