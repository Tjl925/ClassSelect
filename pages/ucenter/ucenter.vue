<template>
  <view class="center">
    ...
    <uni-grid class="grid" :column="3" :showBorder="false" :square="true" style="margin-top: 0px;">
      <uni-grid-item class="item" v-for="(item,index) in gridList">
        <uni-icons class="icon" color="#007AFF" :type="item.icon" size="50" @click="tapGrid(index)"></uni-icons>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      tapGrid(index) {
        // 检查用户是否登录
        if (!store.hasLogin) { // 确保正确调用 hasLogin 方法
          // 用户未登录，显示登录提示或重定向到登录页面
          uni.showModal({
            title: '提示',
...
<style lang="scss" scoped>
  ...
  .userInfo {
    padding-top: 100px;
    background-image: url("https://mp-55788fb7-2392-4142-8486-dfaba6fb925c.cdn.bspapp.com/cloudstorage/info-banner.png");
    flex-direction: column;
    align-items: center;
